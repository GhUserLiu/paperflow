...
async def basic_search_example():
    ...
    try:
        # 配置搜索参数
        search_params = ArxivSearchParams(keywords=["autonomous driving"], max_results=10)
        ...
        successful, failed = await collector.run_manual_collection_async(
            search_params=search_params, download_pdfs=True, use_all_sources=False  # 仅 arXiv
        )
...
async def advanced_search_example():
    ...
    try:
        # 搜索参数
        search_params = ArxivSearchParams(keywords=["deep learning"], max_results=20)
...
async def bilingual_search_example():
    ...
    try:
        # 搜索参数（中英文）
        search_params = ArxivSearchParams(keywords=["智能网联汽车"], max_results=15)
        ...
        # 执行采集（启用多来源）
        successful, failed = await collector.run_manual_collection_async(
            search_params=search_params, download_pdfs=True, use_all_sources=True  # 启用多来源搜索
        )
