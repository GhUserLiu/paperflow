    ...
    async def _prepare_chinaxiv_metadata(self, paper_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                ...
                'published': published_date.strftime('%Y-%m-%d') if isinstance(published_date, datetime) else str(published_date),
                ...
                'categories': [paper_data.get('category', '')] if paper_data.get('category') else [],
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get('success') and data.get('data'):
                        # Parse API response
                        results = data['data'].get('list', []) if isinstance(data['data'], dict) else data['data']

                        for item in results[:search_params.max_results]:
                            paper_data = self._parse_api_result(item)
                            if paper_data:
                                # Apply filters
                                paper_meta = asyncio.run(self._prepare_chinaxiv_metadata(paper_data))
                                if paper_meta:
                                    if self._apply_filters(paper_meta, search_params):
                                        papers.append(paper_meta)
                    else:
                        logger.warning(f"ChinaXiv API returned error: {data.get('message', 'Unknown error')}")
                ...
                logger.warning(f"ChinaXiv API search failed: {str(api_error)}, trying web scraping...")
                papers = self._fallback_web_search(keywords, search_params)
